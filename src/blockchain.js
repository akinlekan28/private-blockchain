...
class Blockchain {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (time > currentTime - 300000) {
                    const success = bitcoinMessage.verify(message, address, signature)
                    if (success) {
                        const block = new BlockClass.Block({"owner": address, "star": star});
                        const result = await self._addBlock(block)
                        resolve(result)
                    } else {
                         reject({ error: "Could not verify sign message" });
                    }
                } else {
                    reject({ error: "Block could not be added due to timeout"});
                }
            } catch (error) {
                reject({error});
            }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
           const blockSearch = self.chain.filter(item => item.hash === hash)
           if(blockSearch){
               resolve(blockSearch[0])
           } else {
               resolve(null);
           }
        });
